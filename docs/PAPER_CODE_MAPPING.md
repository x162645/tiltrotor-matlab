...
| `RESOLVED_HIGH` | 数值计算错误 | `model\rotor_model_bemt.m` | 旧实现只检查数值序列，4° 负推力点可在原始动量残差约 −930 N 时被上层接受 | 已拆分 `coupledConverged`/`physicalConverged`，复算原始闭合残差并沿整机、配平和服务链传播；负推力分支仍明确不支持 |
...
| M-008 | 等效诱导速度迭代 | 5 | 5 of 18 | 式(13) | `model\rotor_model_bemt.m` | `rotor_model_bemt` | `CT,mu,lambda0,lambda1,viTarget,viNew,inducedClosureResidual,physicalConverged` | `CT=max(T,0)/(0.5*rho*A*(Omega*R)^2)`；`lambda0=-Vaxial/(Omega*R)`；`lambda1=lambda0-vi/(Omega*R)`；`viTarget=tipSpeed*CT/(4*sqrt(lambda1^2+mu^2))`；`viNew=0.5*(vi+viTarget)`；迭代后以未截断 `T` 复算动量残差 | 旋翼轴向等效入流 | m/s, N | `CT` 归一化使式(13)与正推力动量关系代数等价；`lambda0` 符号按当前 `Vaxial` 正向映射 | 保留正推力保护和小分母保护；数值收敛与物理闭合分离；负推力/风车分支未增加 | `SIMILAR` | `NUAA_EQ13`; 松弛系数 `0.50`；`check_rotor_physical_convergence` 验证 4° 拒绝和 12° 基准不变 |
