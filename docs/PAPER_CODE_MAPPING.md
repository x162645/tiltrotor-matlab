...
| M-010 | 机翼滑流区面积 | 5 | 5 of 18 | Figure 3；式(16) | `model\wing_model.m` | `wing_model` | `betaPaper,angleRaw,muMean,muRaw,SslipRawHalf,SslipHalf,SfreeHalf` | `betaPaper=pi/2-betaM`; `SslipRawHalf=SslipMaxHalf*(sin(1.386*(pi/2-betaPaper))+cos(3.114*(pi/2-betaPaper)))*((muMax-muMean)/muMax)`；再独立钳位到 `[0,min(SslipMaxHalf,S/2)]` | 半翼滑流/自由流面积 | m^2 | 论文短舱角与代码 `betaM` 相反，先转换再代入式(16)；`muMean` 用左右旋翼当前 `hypot(muLong,muLat)` 平均 | 原始公式结果保存为 `SslipRawHalf`；面积钳位是代码保护，不是论文公式；`SslipMaxHalf=4.0`、`muMax=0.35` 仍为 `ASSUMED_CONCEPT` | `SIMILAR` | `NUAA_EQ16_WITH_PHYSICAL_AREA_GUARD`; 已移除旧 `0.25` 速度衰减和 `0.60` 下限方向因子 |
