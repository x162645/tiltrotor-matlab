...
| M-005 | 叶素局部速度与非均匀诱导速度 | 4-5 | 4-5 of 18 | 式(6)(7)(12) | `model\rotor_model_bemt.m` | `blade_loads` | `UT,UP,viField,VtanTrans,Vrad,beta,betaDot` | `UT=Omega*r+VtanTrans`；`viField=viMean.*(1+cos(psi).*(rMid/R))`；`UP=Vaxial+viField-beta*Vrad-r*betaDot` | 旋翼叶素局部 | m/s | 式(12)按当前空间约定 `psi=0` 沿 `+eD`；左右旋翼使用同一空间 `cos(psi)` 场，`rotDir` 只控制叶片运动相位 | 已在诱导速度-挥舞-叶素载荷迭代内部参与计算；未添加论文外的前进比衰减或悬停淡出 | `SIMILAR` | `NUAA_EQ12_FIRST_HARMONIC`; 仍需人工核对论文旋翼轴系 `psi` 正方向与当前 `+eD` 约定 |
...
| M-008 | 等效诱导速度迭代 | 5 | 5 of 18 | 式(13) | `model\rotor_model_bemt.m` | `rotor_model_bemt` | `CT,mu,lambda0,lambda1,viTarget,viNew` | `CT=max(T,0)/(0.5*rho*A*(Omega*R)^2)`；`lambda0=-Vaxial/(Omega*R)`；`lambda1=lambda0-vi/(Omega*R)`；`viTarget=tipSpeed*CT/(4*sqrt(lambda1^2+mu^2))`；`viNew=0.5*(vi+viTarget)` | 旋翼轴向等效入流 | m/s | `CT` 归一化为当前代码推导定义，使式(13)与正推力动量关系 `T/(2*rho*A*sqrt(Vplane^2+(Vaxial+vi)^2))` 代数等价；`lambda0` 符号按当前 `Vaxial` 正向映射 | 保留正推力保护 `max(T,0)` 和小分母保护；未增加风车/自转分支 | `SIMILAR` | `NUAA_EQ13`; 诱导速度迭代平均系数为 `0.50` |
...
| M-010 | 机翼滑流区面积 | 5 | 5 of 18 | Figure 3；式(16) | `model\wing_model.m` | `wing_model` | `betaMCode,slipstreamAngleArgument,angleRaw,muMean,muRaw,SslipRawHalf,SslipHalf,SfreeHalf` | `slipstreamAngleArgument=pi/2-betaM`; `SslipRawHalf=SslipMaxHalf*(sin(1.386*(pi/2-betaM))+cos(3.114*(pi/2-betaM)))*((muMax-muMean)/muMax)`；再独立钳位到 `[0,min(SslipMaxHalf,S/2)]` | 半翼滑流/自由流面积 | m^2 | 论文式(16)变量 `betaM` 与代码 `betaM` 一致：0 deg 为直升机模式，90 deg 为固定翼模式；Figure 1 的物理短舱角标注采用另一角度基准，不能与公式变量直接混用；`muMean` 用左右旋翼当前 `hypot(muLong,muLat)` 平均 | 原始公式结果保存为 `SslipRawHalf`；面积钳位是代码保护，不是论文公式；`SslipMaxHalf=4.0`、`muMax=0.35` 仍为 `ASSUMED_CONCEPT` | `SIMILAR` | `NUAA_EQ16_WITH_PHYSICAL_AREA_GUARD`; 已移除旧 `0.25` 速度衰减和 `0.60` 下限方向因子；不再输出误导性 `betaPaper` 字段 |
| M-011 | 机翼滑流区局部速度 | 6 | 6 of 18 | 式(17) | `model\wing_model.m` | `one_region` | `Vlocal,wakeVelocity` | 当前仍为 `Vlocal + P.rotor.wakeFactor*max(rotor.inducedVelocity,0)*rotor.eT` | 机体系局部速度 | m/s | 未按式(17)显式实现 `v1d*sin(betaM)` / `-v1d*cos(betaM)` 结构 | 本轮不修改 `P.rotor.wakeFactor`，不实现式(17) | `MISSING_IN_CODE` | 式(17)仍为后续任务 |
