...
Severity is the risk to interpretation or model behavior if the current meaning/value remains unresolved. A missing citation by itself is not `HIGH`. `Blocks` names the first downstream activity that cannot make a defensible aircraft-specific claim without closing the gap. No item below authorizes a parameter change.
...
- Severity: `LOW` for inactive metadata clarity; compatibility fields themselves are `INFO`.
...
There are no `CRITICAL` gaps in this static audit: the conceptual code can exist and run with assumptions. The listed `HIGH` gaps prohibit aircraft-specific interpretation, not basic execution.
