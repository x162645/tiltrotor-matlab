...
> 后续证据闭合说明：NASA TP-2004-212262 给出的 51 点桨叶质量分布现已可推导
> `Sblade=63.09468745 kg m`；它与公开报告列出的
> `Ib=142.36088457 kg m^2` 可组成相容的来源质量属性包。两者不是同一资料中联合报告的一组直接测量，
> 分布积分所得惯量与报告名义值相差 2.32%，因此不能称为严格同源实测。该发现不追改本文件记录的
> V1 冻结过程，也不改变既有正式验模结果；最新证据状态与单因素检验见
> `docs/XV15_SOURCE_CONTRACT_CLOSURE.md`。
...
> 后续证据边界：这里的实现只检查零转速静态几何与输入映射，不检查试验转速下
> 控制链和桨叶受载后的实际桨距。因此这个历史布尔量不再足以单独许可绝对量级验模；
> 它最多表示“静态映射资料齐备”。受载桨距传递与径向分布仍须作为独立证据边界，
> 详见 `docs/XV15_SOURCE_CONTRACT_CLOSURE.md`。
...
> 本节保留的是 V1 冻结前的历史待办，不再代表当前证据状态。质量属性、试验密度、
> 0.75R 桨距定义等项目的后续闭合结果，以
> `docs/XV15_SOURCE_CONTRACT_CLOSURE.md` 为准。
