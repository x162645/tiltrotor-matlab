...
离散网格给出九点中8点可信，但“8/9”不说明未采样区域，也不证明相邻模式定义之间连续。
45 m/s 的 30°/60° 双侧扰动审计显示：30° 双侧可信但直接执行量/状态角最大跳变约为
4.50°；60° 双侧旋翼物理闭合通过、整机配平残差门禁未过，诊断量最大跳变约为11.67°。
因此当前离散可行区不能称为已证实连续的物理转换路径。
...
0.0641—0.0643三类关键量。三项贡献均说明相对已有流程的增量。
""",
    )
    write_csv(
...
关键结果和研究边界排序。两种语言共享8/9、1.5248%、0.0641—0.0643三个定量结论，
均未宣称XV-15复现或整机定量验证。
""",
    )
    write(
