...
但 \(C_T/\sigma\) 的平均绝对误差约为0.0641至0.0643，存在显著幅值偏差。

本文结果支持概念模型在限定工况下的内部一致性、短舱载荷通道辨识和部件级公开数据
关联，不支持XV-15型号复现、飞行安全包线或完整伺服—铰链—结构耦合结论。

![模型能力与最终声明边界](figures/图18_结论适用边界.png)
...
are approximately 0.0641--0.0643, revealing a substantial amplitude bias.

The evidence supports the model's internal consistency. It also supports
mechanism-based nacelle analysis and correlation of component-level outputs with
public data over the stated conditions. It does not establish an XV-15
reproduction, a flight-safety envelope, or a fully coupled
...
当前正式旋翼实现，后者用于同参数交叉比较。二者出现差异时，首先追溯局部速度、诱导闭合、
...
XV-15的构型、操作、风洞、飞行与性能比较资料分别来自公开报告
\cite{nasa62407,dugan1980,nasa100025,nasa81177,nasa166537}，其用途按工况拆分。
旋翼理论补充参考经典螺旋桨、动态入流和直升机动力学文献
\cite{dreier2007,bramwell2001,pitt1981,peters1995,glauert1935,mccormick1995}。
稳定性、辨识、优化与数值计算方法参考
\cite{nelson1998,anderson1999,drela2014,tischler2012,jategaonkar2015,klein2006,golub2013,kelley1999,press2007}。
不确定度与校核框架还比较了工程标准和敏感性方法
\cite{asmevv20,jcgm100,saltelli2008,coleman2009,aiaag077}；这些资料只支持方法选择，
不向当前概念参数传递型号身份。
...
本章把研究对象限定为公开资料约束下的通用低阶模型，并将问题分解为部件载荷闭合、
可信配平、短舱状态化和证据评价。文献可以支撑方程结构、候选参数或局部关联，却不足以
唯一辨识整机。三项贡献因而围绕可追溯建模、左右短舱动态通道和分层可信度展开，而非
追求型号复现。下一章冻结全模型共同使用的坐标、单位、状态和载荷参考点。

# 第二章　坐标系、动力学理论与模型总体架构
...
负推力、诱导迭代失败、NaN、Inf和非预期复数均必须沿调用链返回。对正推力分支，配平门禁同时要求数值序列收敛与原始诱导闭合残差达标；对负推力分支，原载荷可以用于诊断，但在尚无风车/自转闭合时必须拒绝。配平求解器不能把
...
![三个代表点的模型层级特征根](figures/图05_代表点模型层级特征根.png)
...
![外部数据筛选与避免循环论证](figures/外部数据筛选与防止循环论证.png)
...
程序校核、数值收敛、独立实现比较和外部关联回答不同问题。误差被分为参数、模型形式、
数值与数字化四类；缺少可信联合分布时不构造概率置信区间。外部资料进入定量比较前须
满足对象、构型、工况、定义和独立性要求，证据等级只约束具体输出，不能在整机模块间
传递。下一章依照该框架同时报告正面和负面结果。

# 第七章　模型校核与外部关联结果

本章汇总对主要结论有直接约束力的校核和关联结果，重点讨论旋翼公开试验曲线、配平趋势以及九/十三状态共享导数差异。

## 7.1 NASA 悬停旋翼试验数据关联复核

两次独立数字化的逐点差不超过 \(0.003\)。在共同有效总距区间内，当前正式旋翼模型
使用6个点，\(C_T/\sigma\) 的 MAE 为0.06434、RMSE为0.06437、NMAE为0.7229，
最大绝对误差为0.06640，平均绝对相对误差为0.3982。试验斜率为
0.009030/deg，模型斜率为0.009220/deg，方向一致；0°、2°、4°、6°、8°和10°六个计算点失败或
不满足有效门禁。南航公开公式参考模型使用7个有效点，MAE为0.06405、RMSE为0.06408、
NMAE为0.5893，0°至8°共五个点失败。

该结果显示两条未调参路径能够给出同方向推力增长趋势，却存在约0.0641至0.0643的系统
幅值偏差。偏差远大于数字化不确定度，不能归因于取点误差。更合理的解释是桨叶几何、
翼型、扭转、损失修正及试验构型不一致。最终证据表述为“基于公开试验图线的部件级
外部数据关联，趋势一致但幅值偏差显著”，不称为型号验证。

![NASA悬停旋翼推力系数外部关联](figures/旋翼悬停推力系数外部关联.png)
...
本次独立复核分支在MATLAB R2021a上执行28项完整回归，28项均满足现有判据，耗时1777.31 s。
覆盖范围包括质量惯量、短舱端点、旋翼总距单调性、左右对称、南航公开公式闭合、旋翼数值/物理收敛分离、机翼
速度链、旋翼力矩链、一阶谐波挥舞、气动部件、控制架构、十三状态接口/配平/线性化/
研究时域、通用配平来源、模式边界审计、控制分配、可信度诊断、机翼连续混合、速度平方律、旋翼网格
...
外部悬停关联的当前旋翼在0°至10°处失败，其中4°是数值序列收敛但物理闭合不成立；南航参考路径在0°至8°失败。这些点
...
进行线性/非线性和时域比较。后级结果不能反过来证明前级模型在限定检查下内部一致。
...
完整回归与数值检查表明冻结模型可复算；NASA悬停曲线二次数字化显示斜率方向一致但
幅值偏差显著。九/十三状态共享A块差异由左右机翼尾流求值定义引起，集中在滚转/偏航
阻尼导数，不是状态数量本身导致的退化。整机公开资料只支持趋势与缺口识别。本章结果
限定了短舱时域响应能够采用的初值和声明强度。
...
![代表短舱事件的时间步收敛](figures/图09_新增计算时间步收敛.png)
...
三个可信工况中，对称小扰动以纵向通道为主，差动小扰动均产生横航向响应；速度、短舱
角和尾流覆盖调制部件权重。失配、延迟和冻结通过瞬时差动量破坏左右对称。1.5248%的
时间步峰值变化支持当前设置下的数值稳定性，但执行机构与气动参数仍限制幅值解释。
机械卡滞和最不利故障组合超出单向规定运动模型。
...
![75度困难工况的俯仰力矩分解](figures/03_75度俯仰力矩分解.png)

## 9.10 75°/60 m/s 与参数优化边界
...
配平结果是由显式模式标签定义的离散概念模型可行性网格，不等于真实飞行转换走廊。内部边界专项审计在 45 m/s、30° 两侧得到可信解，但最小扰动下直接执行量/状态角最大跳变约为 4.50°；60° 两侧的旋翼物理闭合虽通过，整机配平残差门禁均未通过，诊断量最大跳变约为 11.67°。因此现有证据不能证明三种配平定义在 30°/60° 处连续衔接。
75°/80 m/s在联合优化正式旋翼路径可可信配平，而南航参考旋翼路径在相同条件下失败，
说明高短舱角前飞结论对旋翼模型形式敏感。

![离散速度—短舱角配平可行性网格（归档图名保留“加密转换走廊”）](figures/16_加密转换走廊.png)

![优化前后俯仰力矩分解](figures/18_优化前后俯仰力矩分解.png)
...
其中，移动短舱质量900 kg、质心半径0.75 m以及执行机构带宽4 rad/s、阻尼0.8、
速率20 deg/s和加速度30 deg/s²均为通用概念或研究占位参数。前两项主要缩放重心移动、
惯量变化和力臂效应，后四项主要改变短舱角速度、角加速度与峰值时刻。本文据此把响应
符号和主导通道与峰值幅度分开报告：前者由几何、坐标和左右对称性共同约束，后者必须
随这些占位参数重新评估。该分组给出了优先替换顺序，也避免把单一参数组的峰值误读为
型号载荷。
...
![纵向配平参数归一化敏感性](figures/05_归一化参数敏感性.png)

![参数敏感性矩阵的奇异值分解](figures/07_参数SVD.png)
...
参数审计表明，决定配平和短舱瞬态的若干气动及执行机构量仍是等效参数或研究占位。
联合优化在冻结边界内得到8/9可信点且未使用外部试验数据；75°/40 m/s仍因低动压、
俯仰力矩和升降舵限位共同失败。高短舱角前飞结果对旋翼形式敏感，离散可行区不能称为
真实转换走廊。最终结论按结构性、趋势性和模型敏感定量量归纳。
...
质量统一纳入同一低阶框架。由此可在相同瞬时短舱角下区分速率、延迟和左右不同步造成
的载荷差异，并把刚体响应追溯到直接推力转向、差动力矩、质量属性变化或后续气动反馈。
...
短舱状态化后的首要认识是：对称运动主要通过总推力方向、纵向力矩和质量属性变化进入
纵向通道，差动运动则把左右旋翼载荷和半翼尾流差转换为滚转—偏航耦合；左右带宽、
速率或延迟不一致会把名义对称命令分解出瞬时差动分量。该认识可用于确定后续工作的
优先级：先量化左右短舱同步性与操纵余度，再补充旋翼—半翼干扰和执行机构时历数据，
最后评价峰值载荷。部件载荷分解还指明了试验测量位置与模型升级对象，使有限数据能够
优先约束对结论最敏感的通道。

可信配平门禁的工程作用是阻止把控制触界、残差较大或雅可比病态的点直接用于动态响应。
保留75°/40 m/s失败点并不削弱方法价值，反而给出当前模型和操纵权限共同作用下的可解释
边界，提示高短舱角前飞研究应优先检查尾翼动压、俯仰力矩来源与控制余度。
...
本文完成统一部件模型、可信配平、左右短舱状态扩展和分层可信度分析。核心定量结果是
联合优化模型8/9可信点、代表响应时间步最大变化1.5248%，以及悬停曲线
\(C_T/\sigma\) 约0.0641—0.0643的平均绝对偏差。三项贡献均在通用低阶模型范围内
成立；整机外部定量验证、真实执行机构辨识、非定常干扰和双向结构耦合仍待完成。

# 附录A　公式—代码—参数—测试追溯说明

本附录对应最终追溯表。每个物理模块记录论文公式、原始文献页码与公式/图表号、当前
函数与变量、参数来源分类、单位及坐标变换和覆盖测试。标准理论闭合与文献直接值分开
标识，研究占位、工程假设和未实现项不得记作已核实型号数据。

# 附录B　验证与外部数据矩阵

验证矩阵按接口、质量属性、部件载荷、配平、线性化、短舱动态和外部关联组织。失败、
不可比和待补数据状态均予保留。外部资料矩阵额外记录构型、工况、坐标、原始单位、
数字化过程及是否参与调参，以防止不同来源间的循环论证。

# 附录C　可重复性与数据完整性

计算结果、图表原始数据、生成脚本和构建日志由最终哈希清单关联。旋翼公开试验图线另存
原始页、截图、轴标定和两次独立取点。复算必须使用冻结状态/输入顺序、参数身份和失败
门禁；软件版本、命令、警告与输出哈希随提交包保存。
...
九状态刚体向量严格采用
\([u,v,w,p,q,r,\phi,\theta,\psi]^T\)。前三项是在机体系表达的质心速度，随后三项
为机体系角速度，最后三项为3-2-1欧拉角。十三状态在其后依次增加
\([\beta_L,\beta_R,\dot\beta_L,\dot\beta_R]^T\)。共享前九项使同一配平点能够直接
构造十三状态初值，但不保证共享A块完全相同，因为部件调用路径还可能采用不同左右
定义。

九状态控制向量保持既有总距、差动总距、对称纵向周期、差动纵向周期、短舱角、升降舵
和方向舵顺序。十三状态命令模型把左右短舱命令作为独立输入，并保留原刚体控制的相对
顺序。所有角输入进入三角函数前以rad表示，报告图表可换算为degree。任何后续接口扩展
都必须同步修改配平未知量、B矩阵列名、测试镜像关系和本附录。

## D.2 三个代表点的复算约束

代表点B15/V20、B45/V35和B75/V80分别固定短舱角15°、45°、75°及速度20、35、
80 m/s。初始刚体状态和控制来自冻结可信配平表，十三状态的左右短舱角相等、角速度为
零。重新计算时不得用时域峰值反向调整配平或执行机构参数；若模型、参数或数值步长发生
改变，应生成新版本结果，而不是覆盖本论文冻结值。

共享A块审计必须在相同参数、控制、短舱角和中心差分尺度下进行。逐元素比较包含九个
输入状态，共81个导数；部件分解至少含左右旋翼、机翼、机身、平尾和垂尾。当前差异的
最大元素位于 \(p\) 或 \(r\) 相关导数，主导部件为机翼。共享B块差在舍入量级，不可
用来推断A块也应严格相同。

## D.3 外部旋翼曲线复算约束

NASA TM-86854图25数字化采用PDF第54页（原文第52页），保存原始页和截图。两次取点
的横轴均为总距degree，纵轴为 \(C_T/\sigma\)。采用值取两次均值，比较门禁要求模型
求解成功且总距不低于4°。失败点保留在计数中，不进入MAE分子。改变门禁、插值或失败
处理会改变有效点数，必须同时报告新旧口径。

当前正式旋翼的共同有效点数为6，失败点为0°、2°、4°、6°、8°、10°；南航公开公式参考旋翼的
有效点数为7，失败点为0°、2°、4°、6°、8°。最终采用的MAE分别为0.0643402和
0.0640525，RMSE分别为0.0643727和0.0640844。任何图题和摘要引用均以这些二次复核值
为准。

## D.4 结果表的使用原则

配平表同时保存状态、控制、残差、边界、余度和可信度分类；只复制“成功”列会丢失
物理判据。特征根表必须与对应A矩阵、状态顺序和配平点配套。时域峰值表必须同时记录
事件类型、幅值、执行机构参数、积分时间和时间步。参数表中的来源分类不能因论文语言
精简而删除。

图表是数据的视图，不是新的数据源。所有计算图应能追溯到CSV或MAT文件与生成脚本；
公开试验图线还需追溯到PDF页和轴标定。若图与表发生差异，以冻结数值主表和可重复脚本
为准，并把差异记录为问题，不通过手工修改图形掩盖。

复现实验还应记录软件版本和执行顺序。MATLAB用于重新计算模型与回归，Python只整理
冻结数据、生成文档和清单，XeLaTeX与Biber负责排版和文献。排版脚本不得重新求解配平
或更改数据精度；分析脚本不得改写默认参数。最终PDF中的数字应能回到正文Markdown、
冻结CSV及其生成日志中的同一数值，而不是从图像人工抄录。
复算若产生不同结果，应并列保存旧值、新值、差异、原因与结论影响，禁止静默覆盖。
这样可把科学结论与生成工具解耦，并让后续审阅直接定位变化来源。
所有差异都应可追踪、可解释并可复核。

## D.5 最终声明核对

可以复述的结论包括：模型在现有覆盖工况下内部一致；联合优化概念参数在九个离散工况
中得到八个可信点；三个代表点中对称和差动短舱运动显示不同主导通道；悬停曲线趋势
方向一致但幅值偏差显著。不能复述为XV-15整机复现、型号级稳定性、真实执行机构性能、
飞行安全包线、机械卡滞载荷或完整双向耦合。若引用者省略上述限定，所得结论将超过本
论文证据。
