...
本任务不修改正式物理模型、默认参数、控制限位、可信度门槛或论文冻结文件。0°—10°的失败和负推力点均保留。
...
|隔离 worktree|`E:\tiltrotor-worktrees\low-collective-quick-audit-pr58-20260726`|
|审计分支|`codex/low-collective-quick-audit-pr58-20260726`|
...
|PR 正文记录的旧 head|`19618b2594fe42ac08260c251a55df418c755121`|
|差异|PR 正文 SHA 已过期；本审计只使用实时远程 head `dad5be2f...`|
...
|---:|---|---|---:|---:|---:|---:|---:|---|
|4°|成功，负推力|`CONVERGED`|18|0.0000621|-930.152|-0.003158|-930.152|收敛|
...
|12°|成功，正推力|`CONVERGED`|17|12.1956|16532.180|0.056125|1.414|收敛|

4°虽然被正式路径标记为收敛，但原始 `T_BEMT-T_momentum` 仍为约 -930 N。原因不是隐藏负推力，而是正推力守卫令动量目标为零，固定点只要求 `vi` 衰减到近零。这是“负推力分支未闭合”的直接证据。
...
10. **现在是否要改正式物理模型？** 本任务内不改。证据不足以选择唯一新物理分支。
...
不删除 12°—22°斜率方向结论，也不删除 0°—10°负推力/失败点。没有一句冻结结论需要在本任务中直接改写文件；正式论文修改应另开受控任务。
...
完整 `run_all_checks` 未运行。PR #58 记录的完整回归耗时约 500 s，超过本项目“预计数分钟先确认”的门槛；本任务按用户允许的“新增聚焦测试 + 旋翼相关测试”停止。
