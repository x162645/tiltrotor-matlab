...
for iCheck = 1:numel(checks)
    fprintf('%-64s : %s\n', checks{iCheck}, ...
        ternary(passed(iCheck), 'PASS', 'FAIL'));
    if ~passed(iCheck)
        fprintf('  %s\n', messages{iCheck});
    end
end
fprintf('All nacelle dynamics validation checks passed: %d\n', result.allPassed);
    ...
    function write_markdown_report(resultData, filePath)
        ...
        fprintf(fid, '|---|---:|---:|---:|---|\n');
        for iLegacy = 1:numel(resultData.legacy)
            fprintf(fid, '| %s | %.0f | %.0f | %.3g | %s |\n', ...
                resultData.legacy(iLegacy).caseName, ...
                resultData.legacy(iLegacy).stateDimension, ...
                resultData.legacy(iLegacy).eomLength, ...
                resultData.legacy(iLegacy).maxAbsDiff, ...
                ternary(resultData.legacy(iLegacy).pass, 'PASS', 'FAIL'));
        end
        fprintf(fid, '\n## Actuator Checks\n\n');
        ...
        for iActuator = 1:numel(rows)
            fprintf(fid, '| %s | %.3g | %.3g | %.3g | %.3g | %.3g | %.3g | %s |\n', ...
                rows(iActuator).caseName, rows(iActuator).commandDeg, ...
                rows(iActuator).clampedCommandDeg, rows(iActuator).minBetaDeg, ...
                rows(iActuator).maxBetaDeg, rows(iActuator).maxBetaDotDegPerSec, ...
                rows(iActuator).maxRateStateDegPerSec, ...
                ternary(rows(iActuator).pass, 'PASS', 'FAIL'));
        end
        ...
        for iEquiv = 1:numel(rows)
            fprintf(fid, '| %s | %.3g | %.3g | %.3g | %.3g | %s | %s |\n', ...
                rows(iEquiv).caseName, rows(iEquiv).V, rows(iEquiv).betaMDeg, ...
                rows(iEquiv).maxAbsDiff, rows(iEquiv).relativeDiff, ...
                ternary(rows(iEquiv).pass, 'PASS', 'FAIL'), rows(iEquiv).endpointRisk);
        end
        ...
        for iLinear = 1:numel(rows)
            fprintf(fid, '| %s | %s | %s | %d | %.3g | %s |\n', ...
                rows(iLinear).caseName, rows(iLinear).sizeA, rows(iLinear).sizeB, ...
                rows(iLinear).finiteReal, rows(iLinear).f0Norm, ...
                ternary(rows(iLinear).pass, 'PASS', 'FAIL'));
        end
