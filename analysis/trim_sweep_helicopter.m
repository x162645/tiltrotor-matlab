...
for k = 1:nSpeed
    ...
    if point.status.success
        ...
        point.linearization.A = [];
        point.linearization.B = [];
        point.linearization.report = struct();
        point.linearization.finite = false;
        point.linearization.hasComplex = false;
        point.linearization.hasNaNInf = false;
        point.residualJacobian = empty_jacobian_report(opts);
    end
    ...
    fprintf(['V=%5.1f m/s exitflag=%3d trimNorm=% .3e ' ...
        ...
        'jacRank=%d jacCond=% .3e linFinite=%d status=%s\n'], ...
        ...
        point.anyLimit, point.residualJacobian.rank, ...
        point.residualJacobian.conditionNumber, ...
        point.linearization.finite, point.status.label);
...
sweepReport.allPassed = summary.allTrimSuccessful && ...
    summary.allLinearizationsFinite && summary.noLimitSaturation && ...
...
point = struct( ...
    ...
    'linearization', struct('A', [], 'B', [], 'report', struct(), ...
        'finite', false, 'hasComplex', false, 'hasNaNInf', false), ...
...
function jac = empty_jacobian_report(opts)
jac = struct( ...
...
summary.allLinearizationsFinite = all(arrayfun( ...
    @(p) (~p.status.success) || p.linearization.finite, points));
