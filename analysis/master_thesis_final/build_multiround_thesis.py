...
def collapse_headings(text: str, start: str, end: str, overview: str) -> str:
    """Keep prose in a summary range but replace many short headings by one."""
    i, j = text.find(start), text.find(end)
    if i < 0 or j < 0 or j <= i:
        raise RuntimeError(f"cannot collapse range: {start} -> {end}")
    block = text[i:j]
    block = re.sub(r"^#{2,3}\s+.*$", "", block, flags=re.M)
    return text[:i] + overview + "\n\n" + block.strip() + "\n\n" + text[j:]


def renumber_sections(text: str) -> str:
    """Give every main-chapter section a unique sequential visible number."""
    chapter = 0
    section = 0
    out: list[str] = []
    for line in text.splitlines():
        m_ch = re.match(r"^# 第([一二三四五六七八九十]+)章", line)
        if m_ch:
            zh = m_ch.group(1)
            chapter = {"一": 1, "二": 2, "三": 3, "四": 4, "五": 5,
                       "六": 6, "七": 7, "八": 8, "九": 9, "十": 10}[zh]
            section = 0
            out.append(line)
            continue
        if line.startswith("# 附录"):
            ...
            out.append(line)
            continue
        if chapter and re.match(r"^#{2,3}\s+", line):
            section += 1
            title = re.sub(r"^#{2,3}\s+", "", line)
            title = re.sub(r"^\d+\.\d+\s*[　 ]*", "", title)
            out.append(f"## {chapter}.{section} {title}")
        else:
            out.append(line)
    return "\n".join(out)
...
def structure_round(source: str) -> tuple[str, dict[str, int]]:
    ...
    repeated_evidence_note = (
        "从证据角度看，本小节的方程链首先属于L0理论与实现定义；只有通过有限值、镜像、"
        "端点、力矩臂和多步长检查后，才提升为L1—L2内部证据。对当前通用参数得到的幅值"
        "不得直接冠以XV-15实测含义。"
    )
    repeated_evidence_notes_removed = text.count(repeated_evidence_note)
    text = text.replace(repeated_evidence_note, "")
    removed = 0
    ranges = [
...
"""
    ...
    openings = {
        "# 第一章　绪论": "本章从倾转旋翼机转换飞行的多部件耦合特征出发，界定公开资料条件下可回答的科学问题。输入为公开文献及既有模型能力，输出是研究对象、技术路线和可检验的三项贡献。低阶模型的价值不在于替代高保真分析，而在于以可计算、可追溯的方式分离短舱运动经过推力转向、左右载荷不平衡、移动质量和执行机构反作用进入刚体响应的主要通道，为同步性要求、操纵权限评估和后续试验数据采集提供结构化依据。",
        "# 第二章　坐标系、动力学理论与模型总体架构": "本章建立全文共同使用的坐标、符号和刚体动力学基础。统一定义是后续部件载荷合成、配平以及短舱状态扩展能够相互比较的前提。",
        "# 第三章　部件级非线性飞行动力学模型": "本章把第二章的统一定义落实为旋翼、左右机翼、机身和尾翼的部件级载荷链，最终输出关于实际重心的整机合力与合矩。本章方程链属于理论与实现定义；有限值、镜像、端点、力矩臂和多步长检查分别在第六、七章集中评价，因此不在各小节重复证据等级说明。",
    ...
    info = {
        ...
        "repeated_evidence_notes_removed": repeated_evidence_notes_removed,
    }
    return text, info
...
def technical_round(round2: str) -> str:
    ...
    for marker, summary in CHAPTER_SUMMARIES.items():
        text = insert_before(text, marker, summary)
    text = collapse_headings(
        text, "## 2.1 坐标系与符号", "## 2.2 姿态运动学",
        "## 2.1 理论与模型实现概览",
    )
    text = collapse_headings(
        text, "### 3.1　质量、重心与惯量", "## 3.1 左右旋翼模型的局部速度与几何",
        "## 3.1 部件模型实现概览",
    )
    text = collapse_headings(
        text, "## 4.1 配平问题", "## 4.2 未知量、约束与尺度化残差",
        "## 4.1 配平与线性化流程概览",
    )
    text = collapse_headings(
        text, "## 5.1 从九状态到十三状态", "## 5.2 十三状态、对称与差动坐标",
        "## 5.1 扩展动机与实现概览",
    )
    text = collapse_headings(
        text, "## 6.1 校核、确认与声明等级", "## 6.1 校核、确认与外部关联的边界",
        "## 6.1 可信度方法概览",
    )
    text, _ = remove_between(
        text, "## 7.1 公式与程序校核", "## 7.3 NASA 悬停旋翼试验数据关联复核"
    )
    text, _ = remove_between(
        text, "## 10.1 主要结论", "## 10.1 主要研究结论"
    )
    return renumber_sections(text)
...
"""
    ...
    appendix_a = """# 附录A　公式—代码—参数—测试追溯说明
...
"""
    text = re.sub(r"# 附录A[\s\S]*?(?=# 附录B)", appendix_a, text)
    text = re.sub(r"# 附录B[\s\S]*?(?=# 附录C)", appendix_b, text)
    text = re.sub(r"# 附录C[\s\S]*\Z", appendix_c, text)
    text += r"""
