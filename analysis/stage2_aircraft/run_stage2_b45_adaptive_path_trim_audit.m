function results=run_stage2_b45_adaptive_path_trim_audit(outputRoot)
%RUN_STAGE2_B45_ADAPTIVE_PATH_TRIM_AUDIT
% Diagnostic B45 trim continuation using bounded adaptive path stepping.
% Each target is first attempted directly. On failure, the path step is
% halved; after successful substeps it is allowed to grow again. This tracks
% the same M1 nonlinear solution branch without the exponential cost of
% recursive bisection. No model equations, physical parameters, solver
% tolerances/iteration limits, trim bounds, or trim/control DOFs are changed.
if nargin<1||isempty(outputRoot),outputRoot=fullfile(pwd,'results','stage2_b45_adaptive_path_trim_audit');end
...
P0=P;P0.stage2Numerics.flapInitialLeft=[0.01779756670637974;0.5068067239388294;-0.01925343328660319];P0.stage2Numerics.flapInitialRight=[0.01779756670637973;0.5068067239388293;0.01925343328660322];
p=stage2_evaluate_trim_point('M1_EVIDENCE_V1_PROPAGATION',c,d,z,P0);
assert(p.finiteReal&&p.physicalConverged&&p.physicalBranchSupported&&norm(p.residual-rex)<=5e-9,'B45AdaptivePath:SeedDrift','Archived terminal state failed reproduction.');
fd=[7.5e-5 5e-5 2.5e-5 1e-5 5e-6 2.5e-6 1e-6];fdIdx=1;
trust=2e-2;trustMin=1.25e-4;trustMax=1.6e-1;maxIter=20;minPathDt=2^-14;maxPathAttempts=80;
...
for it=1:maxIter
 ...
 if ~accepted
  ...
  row.stopTag='REJECT_SHRINK';rows(it)=row;fprintf('B45_APATH|it=%d|raw=%.12e|fd=%.7g|trust=%.7g|stop=%s\n',it,raw0,fsel,trust,row.stopTag);continue;
 end
 ...
 fprintf('B45_APATH|it=%d|raw=%.12e->%.12e|phi=%.12e->%.12e|fd=%.7g|cond=%.6e|trust=%.7g->%.7g|a=%.4g|ratio=%.6g|pathAttempts=%g|evals=%d\n',it,raw0,braw,phi0,bphi,fsel,condJ,row.trustRadius,trust,ba,br,batt,totalEval);
 z=bz;p=bp;
end
...
writetable(S,fullfile(outputRoot,'STAGE2_B45_ADAPTIVE_PATH_TRIM_SUMMARY.csv'));writetable(T,fullfile(outputRoot,'STAGE2_B45_ADAPTIVE_PATH_TRIM_ITERATIONS.csv'));results=struct('summary',S,'iterations',T,'finalPoint',p,'finalZ',z,'claimBoundary','ADAPTIVE_PATH_BRANCH_TRACKING_AND_MICRO_FD_ONLY_NO_PHYSICS_PARAMETER_TOLERANCE_BOUND_OR_DOF_CHANGE');save(fullfile(outputRoot,'STAGE2_B45_ADAPTIVE_PATH_TRIM_AUDIT.mat'),'results');disp(S);disp(T);fprintf('B45_APATH_FINAL|start=%.12e|final=%.12e|tol=%.12e|credible=%d|iters=%d|minfd=%.7g|minmargin=%.7g|evals=%d|stop=%s\n',s.startResidualNorm,s.finalResidualNorm,s.trimResidualTolerance,s.credible,s.iterations,s.minimumSelectedFdFraction,s.minimumBoundMargin,s.totalModelEvaluations,s.stopReason);
