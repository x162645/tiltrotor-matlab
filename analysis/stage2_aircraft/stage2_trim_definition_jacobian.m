...
% introduced and no model parameter is changed.
...
for k = 1:numel(alphas)
    ...
    try
        ...
        line(k).residual1 = pt.residual(1);
        line(k).residual2 = pt.residual(2);
        line(k).residual3 = pt.residual(3);
        line(k).z1 = zTrial(1); line(k).z2 = zTrial(2); line(k).z3 = zTrial(3);
        if ~isempty(pt.allocation)
            line(k).cyclicLong = pt.allocation.cyclicLong;
            line(k).elevator = pt.allocation.elevator;
        else
            line(k).cyclicLong = pt.u7(3);
            line(k).elevator = pt.u7(6);
        end
    catch ME
...
audit = struct();
audit.modelIdentity = char(modelIdentity);
audit.caseName = condition.name;
audit.mode = condition.mode;
audit.unknownNames = definition.unknownNames;
audit.baseZ = z;
audit.bounds = definition.bounds;
audit.variableScale = scale;
audit.residualNames = definition.residualNames;
audit.residualScale = residualScale;
audit.baseResidual = base.residual;
audit.baseResidualNormRaw = norm(base.residual);
audit.baseResidualNormScaled = norm(rs);
audit.basePhysicalConverged = base.physicalConverged;
audit.basePhysicalBranchSupported = base.physicalBranchSupported;
audit.basePhysicalStatus = base.physicalStatus;
audit.jacobian = baseResult;
audit.robustness = robustness;
audit.gaussNewtonEtaScaled = eta;
audit.gaussNewtonDeltaZ = dz;
...
audit.fullGaussNewtonStepWithinBounds = alphaBound >= 1;
audit.lineSearch = line;
audit.bestSupportedLineSearchRawResidual = bestRaw;
audit.bestSupportedLineSearchIndex = bestIdx;
...
audit.claimBoundary = ...
    ...
    function out = local_jacobian(stepFraction)
        ...
        for j = 1:3
            ...
            if plusSupported(j) && minusSupported(j)
                J(:,j) = (rp-rm)/(2*h(j));
            end
        end
    ...
    function [r,supported,status] = eval_neighbor(zn)
        ...
        if any(zn < definition.bounds(:,1) | zn > definition.bounds(:,2))
            status = 'OUTSIDE_TRIM_BOUNDS'; return;
        end
        try
            pt = stage2_evaluate_trim_point(modelIdentity,condition,definition,zn,P);
            r = pt.residual;
            supported = pt.finiteReal && pt.physicalConverged && pt.physicalBranchSupported;
...
out.columnNormsRaw = sqrt(sum(J.^2,1));
out.columnNormsScaled = sqrt(sum(Jscaled.^2,1));
if out.finite
    [~,S,V] = svd(Jscaled); s = diag(S); tol = max(size(Jscaled))*eps(max(s));
    out.singularValuesScaled = s(:).'; out.rankToleranceScaled = tol;
    out.rankScaled = sum(s>tol); out.minRightSingularVectorScaled = V(:,end);
    if s(end) <= tol, out.conditionScaled = Inf; else, out.conditionScaled = s(1)/s(end); end
else
    out.singularValuesScaled = [NaN NaN NaN]; out.rankToleranceScaled = NaN;
    out.rankScaled = 0; out.minRightSingularVectorScaled = NaN(3,1); out.conditionScaled = Inf;
end
end
...
for i = 1:numel(z)
    if dz(i) > 0
        alpha = min(alpha,(bounds(i,2)-z(i))/dz(i));
    elseif dz(i) < 0
        alpha = min(alpha,(bounds(i,1)-z(i))/dz(i));
    end
end
