...
The default opt-in `rotDir` mapping makes `lateralCyclic` model-internally effective, but the representative `B_vdot` and raw `dFy` signs can change between betaM conditions. The betaM angle changes the rotor thrust axis `eT`, disk-plane `eD/eY` projections into body axes, proprotor inflow state, flapping response, moment arms, and aerodynamic coupling. Also, B-matrix entries are EOM state-derivative sensitivities, while raw load derivatives are central differences of `[F; M]`; they should not be mixed as identical quantities. The representative points are not a broad trim sweep, so this observation confirms internal response effectiveness only, not external sign validation.
