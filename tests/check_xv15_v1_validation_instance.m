...
expectedChordArea = 0.3623849315068493;
expectedChordR2 = 0.3560002809499333;
expectedChordR3 = 0.3556866438835929;
d2r = pi/180;

%% Partial/default instance: public/reference mappings plus explicit blockers.
...
% The default selected chord is the area candidate only for mapping
% diagnostics. It is not considered frozen until a policy is explicit.
assert(abs(Ppartial.rotor.chord-expectedChordArea) < 1e-12, ...
    'Default area-preserving equivalent chord is incorrect.');
assert(abs(partial.chord.areaPreservingEq_m-expectedChordArea) < 1e-12, ...
    'Area-preserving chord candidate is incorrect.');
assert(abs(partial.chord.hoverThrustR2Eq_m-expectedChordR2) < 1e-12, ...
    'Hover-thrust r^2 chord candidate is incorrect.');
assert(abs(partial.chord.hoverTorqueR3Eq_m-expectedChordR3) < 1e-12, ...
    'Hover-torque r^3 chord candidate is incorrect.');
assert(~partial.chord.policyExplicit, ...
    'Default chord policy must remain unfrozen.');
assert(partial.chord.relativeCandidateSpread > 0.018 && ...
       partial.chord.relativeCandidateSpread < 0.019, ...
    'Chord candidate spread should expose the constant-chord mapping ambiguity.');

% Explicit chord policies must select the intended geometry-only reduction.
sourceR2.chord.reductionPolicy = 'HOVER_THRUST_R2';
[Pr2, mapR2] = build_xv15_v1_hover_validation_instance(P0, struct(), sourceR2);
assert(abs(Pr2.rotor.chord-expectedChordR2) < 1e-12 && ...
    mapR2.chord.policyExplicit, ...
    'Explicit HOVER_THRUST_R2 policy was not applied.');
sourceR3.chord.reductionPolicy = 'HOVER_TORQUE_R3';
[Pr3, mapR3] = build_xv15_v1_hover_validation_instance(P0, struct(), sourceR3);
assert(abs(Pr3.rotor.chord-expectedChordR3) < 1e-12 && ...
    mapR3.chord.policyExplicit, ...
    'Explicit HOVER_TORQUE_R3 policy was not applied.');
...
assert(complete.chord.policyExplicit && ...
    strcmp(complete.chord.selectedPolicy,'AREA_PRESERVING'), ...
    'Complete synthetic contract must freeze its chord reduction policy.');
assert(abs(Pcomplete.rotor.twistTip-syntheticTwistTip) < 1e-12, ...
    '0.75R-anchored linear synthetic twist should be recovered exactly.');
assert(complete.twist.rmsResidual_rad < 1e-12, ...
    'Exact linear synthetic twist should have negligible shape-fit residual.');
assert(complete.twist.weightedRmsResidual_rad < 1e-12, ...
    'Exact linear synthetic twist should have negligible weighted residual.');
...
summary.expectedChordArea_m = expectedChordArea;
summary.expectedChordR2_m = expectedChordR2;
summary.expectedChordR3_m = expectedChordR3;
