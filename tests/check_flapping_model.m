...
add_case('hover zero-cyclic left/right mirror', ...
    test_hover_mirror());
add_case('axisymmetric hover first harmonics vanish', ...
    test_axisymmetric_hover_first_harmonics());
...
fprintf('All flapping model checks passed: %d\n', flapReport.allPassed);
    ...
    function result = test_axisymmetric_hover_first_harmonics()
        [~,~,info] = hover_load([18*d2r;0;0;0;0;0;0], P);
        L = info.rotorLeft;
        R = info.rotorRight;
        ...
        fprintf('\nAxisymmetric hover flapping components [mean cos sin], N*m\n');
        ...
        ok = abs(L.beta1c) < 1e-5 && abs(L.beta1s) < 1e-5 && ...
             abs(R.beta1c) < 1e-5 && abs(R.beta1s) < 1e-5 && ...
             abs(MTL(2)) < 1e-6*max(abs(MTL(1)),1) && ...
             abs(MTL(3)) < 1e-6*max(abs(MTL(1)),1) && ...
             abs(MTR(2)) < 1e-6*max(abs(MTR(1)),1) && ...
             abs(MTR(3)) < 1e-6*max(abs(MTR(1)),1) && ...
             norm(RL) < 1e-6*max(abs(MTL(1)),1) && ...
             norm(RR) < 1e-6*max(abs(MTR(1)),1);
        result = make_result(ok, ...
            'Axisymmetric hover produced nonzero first-harmonic flapping or residual.');
    end

    function result = test_positive_cyclic_direction()
        [~,~,base] = hover_load([18*d2r;0;0;0;0;0;0], P);
        [Fplus,~,plus] = hover_load([18*d2r;0;2*d2r;0;0;0;0], P);
        dF = Fplus - base.F;
        L = plus.rotorLeft;
        R = plus.rotorRight;
        eD = L.eD;
        ok = L.beta1c < 0 && R.beta1c < 0 && ...
             abs(L.beta1c) > 3*abs(L.beta1s) && ...
             abs(R.beta1c) > 3*abs(R.beta1s) && ...
             dot(L.nDisk,eD) > 0 && dot(R.nDisk,eD) > 0 && ...
             abs(dF(2)) <= 1e-8*max(norm(dF),1) + 1e-7 && ...
             dF(1) > 0;
        msg = ['Expected positive common cyclicLong to produce common ' ...
            'beta1c<0, disk normals toward +eD, cancelling lateral force, ' ...
            'and positive longitudinal force.'];
    ...
    function result = test_diff_cyclic_mapping()
        ...
        ok = rightErr < 1e-12 && leftErr < 1e-12 && ...
             ...
             dM(3) < 0 && ...
             abs(dF(1)) <= 1e-8*max(norm(dF),1) + 1e-7;
        result = make_result(ok, ...
