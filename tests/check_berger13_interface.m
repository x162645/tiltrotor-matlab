...
% Independent left/right rotor angles must be active.  The corrected PR3
% path reevaluates every component about the reconstructed actual total CG;
% the average-angle stack is retained for diagnostics only.
...
independentOK = asymInfo.usedIndependentRotorAngles && ...
    ~asymInfo.usedAverageNonRotorAero && ...
    strcmp(asymInfo.forceMomentReference,'ACTUAL_TOTAL_CG') && ...
    norm(asymInfo.momentReferenceCG-asymInfo.massProperties.cgShift) < 1e-12 && ...
    ~isempty(asymInfo.warnings) && ...
    norm(asymInfo.rotorLeft.deltaFromAverage.F) > 1 && ...
    norm(asymInfo.rotorRight.deltaFromAverage.F) > 1 && ...
    all(isfinite([Fasym;Masym]));
add_case('independent loads use actual total CG',independentOK, ...
    'Independent loads or the corrected actual-CG contract was not observable.');
